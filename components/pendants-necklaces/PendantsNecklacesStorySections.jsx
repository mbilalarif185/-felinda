import { Gem, Palette, ShieldCheck, Sparkles } from "lucide-react";

import {
  pendantsNecklacesProcessFooter,
  pendantsNecklacesProcessSteps,
  pendantsNecklacesProcessSubtitle,
  pendantsNecklacesProcessTitle,
  pendantsNecklacesWhyLead,
  pendantsNecklacesWhyPillars,
  pendantsNecklacesWhyTitle,
} from "@/lib/content/pendants-necklaces-page";

import PendantsNecklacesFaqAccordion from "@/components/pendants-necklaces/PendantsNecklacesFaqAccordion";

const WHY_ICONS = [Sparkles, Gem, Palette, ShieldCheck];

function SectionShell({ children, className = "" }) {
  return <div className={`mx-auto max-w-[1040px] ${className}`}>{children}</div>;
}

export default function PendantsNecklacesStorySections() {
  return (
    <div className="mt-14 space-y-16 md:space-y-20">
      <section aria-labelledby="pendants-necklaces-why-heading">
        <SectionShell>
          ...
          <ul className="mt-12 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:gap-6 xl:grid-cols-4">
            {pendantsNecklacesWhyPillars.map((pillar, i) => {
